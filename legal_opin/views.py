...
from .models import LegalEntity, Deal, ExecutiveBody, Participant, DealParticipant, Collegial_governing_bodies, DealParticipant, Collateral
from .forms import LegalEntityForm, ExecutiveBodyForm, ParticipantForm, Collegial_governing_bodiesForm, DealForm, DealParticipantForm, CollateralForm, BaseCollegialBodiesFormSet
...
from legal_opin.util import EgrulDownloader, EgripParser, Converter, Date_Converter, compare_formset_data

# НАСТРОЙКИ: Роли, которые считаются должниками
# Для изменения логики определения должников - отредактируйте этот список
DEBTOR_ROLE_NAMES = ['Заемщик', 'Принципал']
...
class LegalEntityDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        deals_with_data = []
        for deal in deals:
            # Получаем всех должников для данной сделки используя настройку ролей
            from .models import Role_in_deal
            debtor_roles = Role_in_deal.objects.filter(name__in=DEBTOR_ROLE_NAMES)
            borrowers = DealParticipant.objects.filter(
                deal=deal,
                role__in=debtor_roles
            ...
            deal.current_entity_roles = [dp.role.name for dp in current_entity_roles]
            deals_with_data.append(deal)

        context['deals'] = deals_with_data
        
        # Добавляем все роли из БД для фильтра
        from .models import Role_in_deal
        context['all_roles'] = Role_in_deal.objects.all().order_by('name')
...
def check_inn_file(request):
    ...
    if request.method == 'POST' and request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        ...
        if not re.match(r'^\d{10}$', inn):  # Проверяем формат ИНН (10 цифр)
            print('Некорректный ИНН')
            return JsonResponse({'error': 'ИНН должен содержать 10 цифр'})  #если формат некорректный - возвращаем сообщение об ошибке
        ...
        if egrul_vipiska == 'Ошибка':
            ...
            conv = Converter(itog)
            new_mass = conv.itog()
            if new_mass[0]['legal_form'] == 'АО' or new_mass[0]['legal_form'] == 'ООО' or new_mass[0]['legal_form'] == 'ПАО':
...
class LegalEntityCreateView(CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        CollegialFormSet = forms.formset_factory(
            Collegial_governing_bodiesForm,
            extra=0,
            can_delete=True
        )
    ...
    def form_valid(self, form, executive_formset, participant_formset, collegial_formset):
        ...
        for coll_form in collegial_formset:
            if coll_form.has_changed():
                colleg = coll_form.save(commit=False)
                colleg.legal_entity = main_obj
                colleg.save()
...
class LegalEntityUpdateView(UpdateView):
    ...
    #     print('Редактируемая запись:', obj)
    #     # Можно вывести значения полей: print(obj.field1, obj.field2)
    #     print(obj.confirmation_procedure)
    #     return obj
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            print('Основная форма, валидация пройдена')
            if executive_formset.is_valid():
                print('форма по ЕИО валидация пройдена')
                if participant_formset.is_valid():
                    print('форма по участникам валидация пройдена')
                    if collegial_formset.is_valid():
                        print("форма по КОУ валидация пройдена")
                        return self.form_valid(form, executive_formset, participant_formset, collegial_formset)
                    else:
...
class LegalEntityUpdateFromEGRULView(UpdateView):
    ...
    def form_valid(self, form, executive_formset, participant_formset, collegial_formset):
        ...
        print(f"Executive forms: {len(executive_formset.forms)}; will save: {len(executives_to_save)}")
        ...
        print(f"Participant forms: {len(participant_formset.forms)}; will save: {len(participants_to_save)}")
        Participant.objects.filter(legal_entity=main_obj).delete()
        for participant in participants_to_save:
            participant.pk = None
            participant.save()
        print(f"Saved participants count: {Participant.objects.filter(legal_entity=main_obj).count()}")
...
def add_multiple_participants(request, deal_pk):
    """
    Добавление нескольких участников в сделку через AJAX
    """
    if request.method == 'POST':
        try:
            ...
            # Получаем JSON данные с участниками и их ролями
            participants_json = request.POST.get('participants')
            if not participants_json:
                return JsonResponse({'status': 'error', 'message': 'Не переданы данные участников'})

            participants_data = json.loads(participants_json)

            if not participants_data:
                return JsonResponse({'status': 'error', 'message': 'Не выбраны участники'})

            # Сначала проверяем ВСЕ участников на ошибки валидации
            validation_errors = {}  # {entity_id: error_message}
            ...
            for participant_data in participants_data:
                ...
                entity = get_object_or_404(LegalEntity, pk=entity_id)

                # Получаем объект роли по названию
                role_obj = Role_in_deal.objects.filter(name=role).first()
                if not role_obj:
                    validation_errors[entity_id] = f'Роль "{role}" не найдена в системе'
                    continue
                
                # Проверяем, не является ли уже участником с такой же ролью
                if DealParticipant.objects.filter(deal=deal, legal_entity=entity, role=role_obj).exists():
                    validation_errors[entity_id] = 'Юр. лицо с такой ролью уже добавлено в сделку'

            # Если есть ЛЮБЫЕ ошибки валидации - НЕ добавляем НИКОГО
            ...
            # Если валидация прошла успешно - добавляем ВСЕХ участников
            created_count = 0
            with transaction.atomic():
                for participant_data in participants_data:
                    entity_id = participant_data.get('entity_id')
                    ...
                    # Создаем участника (валидация уже пройдена)
                    DealParticipant.objects.create(
                        deal=deal,
                        legal_entity=entity,
                        role=role_obj
                    )
            ...
            message = f'Успешно добавлено {created_count} участников'
...
class DealListView(ListView):
    ...
    def get_queryset(self):
        queryset = super().get_queryset().prefetch_related('participants__legal_entity', 'credit_product')
        ...
        if credit_product:
            queryset = queryset.filter(credit_product__name__icontains=credit_product)
    ...
    def get_context_data(self, **kwargs):
        ...
        # Добавляем информацию о должниках для каждой сделки
        deals = context['deals']
        for deal in deals:
            # Получаем всех должников для данной сделки используя настройку ролей
            from .models import Role_in_deal
            ...
            # Добавляем всех участников для фильтрации по ИНН/ОГРН
            deal.all_participants = [dp.legal_entity for dp in deal.participants.select_related('legal_entity').all()]
        ...
        # Добавляем все кредитные продукты для фильтра
        from .models import Credit_product
        context['credit_products'] = Credit_product.objects.all().order_by('name')
...
class DealDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        deal.credit_product  # предзагружаем credit_product

        # Получаем всех юридических лиц для модального окна добавления участников
        # Показываем все юр.лица, так как одно лицо может иметь несколько ролей
        context['available_entities'] = LegalEntity.objects.order_by('name')

        # Группировка обеспечения
        collaterals = self.object.collaterals.select_related('owner').order_by('owner__name')
        grouped_collaterals = defaultdict(lambda: defaultdict(lambda: {'count': 0, 'is_countable': False}))

        for collateral in collaterals:
            owner_name = collateral.owner.name
            type_display = collateral.get_collateral_type_display()

            group = grouped_collaterals[owner_name][type_display]
            group['count'] += 1
            if collateral.collateral_type == 'RE':
                group['is_countable'] = True

        # Преобразование defaultdict в обычный dict для шаблона
        context['grouped_collaterals'] = {k: dict(v) for k, v in grouped_collaterals.items()}
        context['now'] = datetime.now().date()
        # Пока заглушка, можно будет добавить логику позже
        context['risks'] = []
        
        # Добавляем все роли из БД для модального окна
        from .models import Role_in_deal
        context['all_roles'] = Role_in_deal.objects.all().order_by('name')
...
def add_collateral(request, deal_pk):
    deal = get_object_or_404(Deal, pk=deal_pk)
    CollateralFormSet = modelformset_factory(
        Collateral,
        form=CollateralForm,
        extra=1,
        can_delete=False
    ...
    if request.method == 'POST':
        formset = CollateralFormSet(request.POST, request.FILES, queryset=Collateral.objects.none())
        if formset.is_valid():
        ...
        formset = CollateralFormSet(queryset=Collateral.objects.none())

    context = {
        'formset': formset,
        'deal': deal,
    }
    return render(request, 'legal_opin/collateral_form.html', context)